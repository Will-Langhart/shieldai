...
async function testPineconeCorrectEnv() {
  console.log('🧪 Pinecone Test with Correct Environment Format\n');
  
  try {
    // Use the correct environment format for your serverless index
    const apiKey = 'pcsk_6La4fD_KxUwby6c62E93h7pPAdgjhSG5szTKgqh9Pych3nSwCPVx1Ze5mFEzuFixbqszw1';
    const environment = 'us-east-1-aws'; // Correct format for your index
    ...
    if (stats.totalVectorCount === 0) {
      ...
      const testId = `correct_env_test_${Date.now()}`;
      ...
      const upsertResult = await index.upsert([{
        ...
        values: testVector,
        metadata: {
          content: 'Correct environment test message for Shield AI',
          role: 'user',
          conversationId: 'correct_env_test_conversation',
          userId: 'correct_env_test_user',
